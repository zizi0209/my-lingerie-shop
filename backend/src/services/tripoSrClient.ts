...
const TRIPOSR_API_MODE = process.env.TRIPOSR_API_MODE || 'fastapi';
...
interface TripoSrResult {
 ...
 export async function generateModel3D(imageBuffer: Buffer): Promise<TripoSrResult> {
   try {
    ...
    const generateRes = await fetch(`${TRIPOSR_ENDPOINT}/api/generate`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        image: dataUri,
    ...
    if (!generateRes.ok) {
      ...
      return { success: false, glbBuffer: null, error: `TripoSR API error: ${generateRes.status} - ${errText}` };
    }

    const result = await generateRes.json() as { success?: boolean; glbBase64?: string; error?: string };
    if (!result.success || !result.glbBase64) {
      return { success: false, glbBuffer: null, error: result.error || 'TripoSR returned empty result' };
    }

    const glbBuffer = Buffer.from(result.glbBase64, 'base64');
    if (glbBuffer.length < 100) {
      return { success: false, glbBuffer: null, error: 'GLB file too small, likely invalid' };
    }
 ...
 export async function isTripoSrAvailable(): Promise<boolean> {
   try {
    const healthPath = TRIPOSR_API_MODE === 'gradio' ? '/api/' : '/api/health';
    const res = await fetch(`${TRIPOSR_ENDPOINT}${healthPath}`, {
